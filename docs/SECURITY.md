# Artofix 2.5 — модель безопасности
...
## 6. Обновления 2.5: безопасность и антидетект

### 6.1 Безопасность обхода Cloudflare / Claude
* Инъекция скрипта страницы (`PAGE_INIT_TEMPLATE`) строго сохраняет запрет на `eval`,
  `innerHTML` и строковую конкатенацию: параметры подставляются сериализованным JSON
  через CDP `Page.addScriptToEvaluateOnNewDocument`.
* Эмуляция `chrome.csi`, `chrome.loadTimes`, `chrome.app` и PDF-плагинов защищена
  через `Object.defineProperty` с замороженными прототипами и `Function.prototype.toString`
  маскировкой под `[native code]`.
* Эмуляция `document.hasFocus()` и `visibilityState` предотвращает отсев ботов
  без деградации безопасности песочницы Chrome.

### 6.2 WARP и Чебурнет в безопасной среде
* Управление чистыми IP Cloudflare WARP строго валидирует адреса через `parseIPv4` / `sanitizeHost`.
  Команды `warp fix` и `warp test` не вызывают произвольные shell-скрипты или bat-файлы,
  а используют системный `netsh.exe` со строгим массивом аргументов `spawnFile` без оболочки shell.
* DNS-адреса проверяются строгим регулятором IPv4 (0..255).

### 6.3 Безопасность установщика (Setup) и устранение 0-day векторов
* **Аппаратный чекер**: сбор ядер (`os.cpus()`) и ОЗУ (`os.totalmem()`, `os.freemem()`) выполняется в изолированном main-процессе без вызова внешних утилит командной строки.
* **Защита от MITM / SSRF / подмены источников**: скачивание Python разрешено только с официального домена `www.python.org` по протоколу `https:` со строгой регулярной валидацией пути `sec.sanitizePythonDownloadUrl()`. Любые редиректы на сторонние домены или по незашифрованным протоколам немедленно блокируются.
* **Защита от загрузочных бомб и подмены бинарников**: максимальный размер загрузки ограничен 60 МБ. Скачанный файл проверяется функцией `sec.validateInstallerBinary()` на минимальный размер (15 МБ) и валидный заголовок PE-бинарника (сигнатура `MZ`). Запуск скриптов, командных файлов или повреждённых архивов полностью исключён.
* **Защита от Command / Shell Injection**: запуск инсталлера Python и модулей pip производится исключительно через `execFile` со статическим списком аргументов и `windowsHide: true`. Флаг `shell: true` категорически запрещён.
* **Безопасная очистка**: временный файл инсталлера создаётся в `os.tmpdir()` с правами `0o600` и гарантированно удаляется в блоке `finally` сразу по окончании процесса установки (или при ошибке).
* **Контроль пользователя**: скачивание и запуск компонентов происходят только после явного подтверждения пользователем через диалоговое окно в UI (`setup.html`).
