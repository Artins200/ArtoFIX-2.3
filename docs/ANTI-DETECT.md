...
(тумблеры «умное ожидание», «мягкий вход», «ждать проверку»). Хранятся в
`config.json` → `cf: { enabled, soft_landing, wait_challenge, challenge_timeout,
max_retries }`, применяются при следующем запуске браузера. `challenge_timeout`
ограничен 5–90 с, `max_retries` — 0–3, поэтому окно не «зависает» на проверке.
